...
namespace DB {
  ...
  Table::insert(int x, int y)
  {
    ...
    auto rootSplit = BTrie::lookup(mRootPID, x, rootLID, rootPos);
    ...
    if (rootSplit.isNoChange()) {
      ...
      auto subSplit = BTrie::lookup(subPID, y, subLID, subPos);
    ...
    // Otherwise the lookup caused an insertion.
    ...
    page_id subLID; int subPos;
    BTrie::lookup(newLID, y, subLID, subPos);
